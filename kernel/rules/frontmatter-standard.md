...
client: taste               # client slug; empty (~) when it is own/internal work
...
- **`client`** - client slug (e.g. `taste`); EMPTY (`~`) when the artifact is own/internal. Explicit here (not only in `path`) for self-sufficiency and filtering. A finer `project` field may be added later if needed.
